...
1. Если задача связана с API Яндекс Трекера, всегда проверяй валидность плана по официальному клиенту в папке yandex_tracker_client
1. **Создай детальный план** в папке `.agentic-planning/plan_{краткое_описание_большой задачи}/`
...
1. **Согласуй детальный план** с пользователем, внеси правки
1. **После каждого этапа ОБЯЗАТЕЛЬНО:**
   - ✅ Полностью собрать проект и провалидировать: `npm run validate`, исправить ошибки
   - ✅ Отметить прогресс в файлах плана
   - ✅ Коммит с описанием этапа на русском языке (если ветка `main` или `master` - спроси разрешение)
   - ✅ Пуш в репозиторий
   ...
   - ✅ Если финальная валидация показывает, что весь план успешно выполнен, после завершения последнего шага его надо удалить.
