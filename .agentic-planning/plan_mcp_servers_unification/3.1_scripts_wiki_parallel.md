# Этап 3.1: Служебные скрипты для Yandex Wiki (parallel)

**Время:** ~1-2 часа
**Зависимости:** Этап 2.1 (smoke тесты wiki)
**Параллельно с:** 3.2 (TickTick скрипты)

---

## Задача

Добавить служебные скрипты в yandex-wiki:
1. `scripts/generate-tool-index.ts`
2. `scripts/validate-tool-registration.ts`

---

## Чеклист

### Скрипты
- [ ] Создать `scripts/generate-tool-index.ts`
- [ ] Создать `scripts/validate-tool-registration.ts`

### package.json
- [ ] Добавить `"prebuild": "npm run generate:index"`
- [ ] Добавить `"generate:index": "tsx scripts/generate-tool-index.ts"`
- [ ] Добавить `"validate:tools": "tsx scripts/validate-tool-registration.ts"`

### Валидация
- [ ] `npm run generate:index` работает
- [ ] `npm run validate:tools` работает
- [ ] `npm run build` включает prebuild

---

## Источники для копирования

**Из yandex-tracker:**
- `scripts/generate-tool-index.ts`
- `scripts/validate-tool-registration.ts`

---

## generate-tool-index.ts

### Адаптации

```typescript
// Константы для Wiki
const TOOLS_DIR = path.join(__dirname, '../src/tools');
const OUTPUT_FILE = path.join(TOOLS_DIR, 'generated-index.ts');
const TOOL_FILE_PATTERN = /\.tool\.ts$/;

// Префикс инструментов Wiki
const TOOL_PREFIX = 'yw_';  // или какой используется
```

### Что делает скрипт

1. Сканирует `src/tools/` рекурсивно
2. Находит все файлы `*.tool.ts`
3. Генерирует `generated-index.ts` с экспортами всех Tool классов

### Пример вывода

```typescript
// generated-index.ts - АВТОГЕНЕРИРОВАННЫЙ ФАЙЛ
// Не редактируйте вручную! Запустите: npm run generate:index

export { PingTool } from './helpers/ping/ping.tool.js';
export { GetPageTool } from './api/pages/get/get-page.tool.js';
export { CreatePageTool } from './api/pages/create/create-page.tool.js';
// ... все tools
```

---

## validate-tool-registration.ts

### Адаптации

```typescript
// Импорты для Wiki
import { TOOL_CLASSES } from '../src/composition-root/definitions/tool-definitions.js';

// Путь к tools
const TOOLS_DIR = path.join(__dirname, '../src/tools');
```

### Что проверяет скрипт

1. Все `*.tool.ts` файлы найдены
2. Каждый Tool класс добавлен в `TOOL_CLASSES`
3. Каждый Tool имеет static `METADATA`
4. Каждый Tool имеет метод `getParamsSchema()`

### Пример вывода

```
🔍 Проверка регистрации инструментов...

✅ Найдено 18 файлов *.tool.ts
✅ Зарегистрировано 18 классов в TOOL_CLASSES

✅ Все инструменты корректно зарегистрированы!
```

---

## Изменения в package.json

```json
{
  "scripts": {
    "prebuild": "npm run generate:index",
    "generate:index": "tsx scripts/generate-tool-index.ts",
    "validate:tools": "tsx scripts/validate-tool-registration.ts"
  }
}
```

---

## Валидация

```bash
cd packages/servers/yandex-wiki

# Генерация индекса
npm run generate:index
# Проверить что создан src/tools/generated-index.ts

# Валидация регистрации
npm run validate:tools
# Должно вывести: "Все инструменты корректно зарегистрированы!"

# Полная сборка (включает prebuild)
npm run build
```

---

## Коммит

```
feat(wiki): добавить служебные скрипты

Добавлены скрипты:
- generate-tool-index.ts - автогенерация индекса tools
- validate-tool-registration.ts - проверка регистрации tools

Обновлён package.json:
- Добавлен prebuild для автогенерации
- Добавлена команда validate:tools
```
