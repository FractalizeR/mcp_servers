...
- [ ] 6 tools созданы (get_projects, get_project, get_project_tasks, create, update, delete)
- [ ] Каждый tool имеет schema, metadata, tool файлы
- [ ] ResponseFieldFilter применяется для экономии контекста
- [ ] Metadata содержит category, subcategory, priority, tags
- [ ] index.ts экспортирует все tools
