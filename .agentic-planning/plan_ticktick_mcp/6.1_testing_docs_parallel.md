...
- [x] Unit тесты для OAuth client (11 tests passing)
- [ ] Unit тесты для operations (coverage > 80%) — отложено
- [ ] Unit тесты для tools (coverage > 80%) — отложено
- [ ] Integration тесты — отложено
- [x] Smoke тесты (DI Container + MCP Server Lifecycle - 18 tests)
- [x] README.md создан
- [x] CLAUDE.md создан
...
**Примечание:**
- Smoke тесты (18 тестов) проверяют инициализацию DI контейнера и жизненный цикл MCP сервера
- Smoke тесты требуют запуска из корня monorepo: `npx vitest run tests/smoke --dir=packages/servers/ticktick`
- Полное покрытие тестами (operations, tools, integration) отложено на будущие итерации
