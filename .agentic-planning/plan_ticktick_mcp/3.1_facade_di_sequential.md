...
- [x] types.ts содержит все Symbol токены
- [x] TickTickFacade делегирует все операции
- [x] Convenience методы (getAllTasks, searchTasks, etc.) работают
- [x] operation-definitions.ts экспортирует все операции
- [x] container.ts создаёт DI контейнер
- [x] Все bindings корректны
- [x] `npm run typecheck` проходит
- [x] `npm run lint:quiet` проходит
- [x] `npm run test:quiet` проходит (11 tests)
