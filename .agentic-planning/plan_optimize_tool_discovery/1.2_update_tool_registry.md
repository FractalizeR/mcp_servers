...
- [x] Добавить приватный метод `sortByPriority(tools: BaseTool[]): BaseTool[]`
- [x] Реализовать логику сортировки: `critical → high → normal → low`
- [x] Вторичная сортировка по алфавиту (name) для стабильности
...
- [x] Обновить `getDefinitions()` — применить `sortByPriority` перед возвратом
- [x] Обновить `getEssentialDefinitions()` — применить `sortByPriority`
- [x] Обновить `getDefinitionsByMode()` — использовать отсортированные данные
...
- [x] Логировать порядок инструментов после сортировки (debug level)
- [x] Показывать распределение по priority
...
- [x] Тест: инструменты отсортированы по priority (critical первые)
- [x] Тест: в рамках одного priority — алфавитная сортировка
- [x] Тест: undefined priority трактуется как 'normal'
- [x] Тест: сортировка работает в eager mode
- [x] Тест: сортировка работает в lazy mode

### 5. Валидация

- [x] Запустить `npm run test --workspace=@mcp-framework/core`
