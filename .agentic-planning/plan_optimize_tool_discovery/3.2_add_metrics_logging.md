# Этап 3.2: Добавить метрики и логирование

**Цель:** Логировать размер tools/list response и распределение по категориям/приоритетам

**Затронутые файлы:** `packages/servers/yandex-tracker/src/index.ts`

**Время:** 1 час

**Зависимости:** Требует завершения этапов 1.2, 2.1, 2.2, 3.1

---

## Чек-лист задач

### 1. Добавить функцию подсчёта метрик

**Файл:** `packages/servers/yandex-tracker/src/index.ts`

- [x] Создать функцию `calculateToolsMetrics(definitions: ToolDefinition[])`
- [x] Подсчитать общее количество инструментов
- [x] Подсчитать размер descriptions (символы)
- [x] Оценить количество токенов (symbols / 4)
- [x] Подсчитать распределение по categories
- [x] Подсчитать распределение по priorities
- [x] Подсчитать распределение по subcategories

**Пример функции:**
```typescript
interface ToolsMetrics {
  totalTools: number;
  descriptionLength: number;
  estimatedTokens: number;
  byCategory: Record<string, number>;
  byPriority: Record<string, number>;
  bySubcategory: Record<string, number>;
}

function calculateToolsMetrics(definitions: ToolDefinition[]): ToolsMetrics {
  const descriptionLength = definitions.reduce(
    (sum, def) => sum + def.description.length,
    0
  );

  const byCategory: Record<string, number> = {};
  const byPriority: Record<string, number> = {};
  const bySubcategory: Record<string, number> = {};

  for (const def of definitions) {
    byCategory[def.category] = (byCategory[def.category] || 0) + 1;
    const priority = def.priority || 'normal';
    byPriority[priority] = (byPriority[priority] || 0) + 1;
    if (def.subcategory) {
      bySubcategory[def.subcategory] = (bySubcategory[def.subcategory] || 0) + 1;
    }
  }

  return {
    totalTools: definitions.length,
    descriptionLength,
    estimatedTokens: Math.ceil(descriptionLength / 4),
    byCategory,
    byPriority,
    bySubcategory,
  };
}
```

### 2. Обновить ListToolsRequest handler

**Файл:** `packages/servers/yandex-tracker/src/index.ts` (строки 65-100)

- [x] Получить определения от ToolRegistry
- [x] Рассчитать метрики через `calculateToolsMetrics()`
- [x] Логировать базовую информацию (info level)
- [x] Логировать детальные метрики (debug level)

**Пример логирования:**
```typescript
server.setRequestHandler(ListToolsRequestSchema, () => {
  logger.info(`📋 Запрос tools/list от клиента`);

  const definitions = toolRegistry.getDefinitionsByMode(
    config.toolDiscoveryMode,
    config.essentialTools
  );

  const metrics = calculateToolsMetrics(definitions);

  // Info level: базовая информация
  logger.info(
    `✅ Возвращаем ${metrics.totalTools} инструментов (режим: ${config.toolDiscoveryMode})`,
    {
      totalTools: metrics.totalTools,
      mode: config.toolDiscoveryMode,
      descriptionLength: metrics.descriptionLength,
      estimatedTokens: metrics.estimatedTokens,
    }
  );

  // Debug level: детальная разбивка
  logger.debug('📊 Распределение инструментов', {
    byCategory: metrics.byCategory,
    byPriority: metrics.byPriority,
    bySubcategory: metrics.bySubcategory,
  });

  // Порядок инструментов (для отладки сортировки)
  logger.debug('🔢 Порядок инструментов:', {
    order: definitions.map(d => ({
      name: d.name,
      category: d.category,
      priority: d.priority || 'normal',
    })),
  });

  return {
    tools: definitions.map((def) => ({
      name: def.name,
      description: def.description,
      inputSchema: def.inputSchema,
    })),
  };
});
```

### 3. Добавить предупреждения

**Файл:** `packages/servers/yandex-tracker/src/index.ts`

- [x] Предупреждение если `eager` mode и много инструментов
- [x] Предупреждение если descriptions слишком длинные

**Пример:**
```typescript
// Рекомендация переключиться на lazy mode
if (config.toolDiscoveryMode === 'eager' && metrics.totalTools > 30) {
  logger.warn('⚠️  Рекомендация: много инструментов в eager mode', {
    totalTools: metrics.totalTools,
    estimatedTokens: metrics.estimatedTokens,
    recommendation: 'Рассмотрите TOOL_DISCOVERY_MODE=lazy для экономии контекста',
  });
}

// Предупреждение о больших descriptions
if (metrics.estimatedTokens > 200) {
  logger.warn('⚠️  Descriptions занимают много токенов', {
    estimatedTokens: metrics.estimatedTokens,
    recommendation: 'Сократите descriptions для экономии контекста LLM',
  });
}
```

### 4. Тесты

**Файл:** `packages/servers/yandex-tracker/tests/mcp/tools-metrics.test.ts` (создан)

- [x] Тест: `calculateToolsMetrics()` корректно подсчитывает метрики
- [x] Тест: метрики включают все категории
- [x] Тест: метрики включают все приоритеты
- [x] Мок логгера для проверки вызовов

### 5. Интеграционный тест

- [x] Запустить сервер локально
- [x] Проверить логи при `tools/list` запросе
- [x] Убедиться что метрики логируются корректно
- [x] Проверить что порядок инструментов соответствует priority

**Команда для тестирования:**
```bash
# В одном терминале
npm run start:server --workspace=mcp-server-yandex-tracker

# В другом терминале
node packages/servers/yandex-tracker/scripts/test-tools-list.mjs
```

### 6. Валидация

- [x] Запустить `npm run test --workspace=@mcp-server/yandex-tracker` (595 тестов ✅)
- [x] Запустить `npm run build --workspace=@mcp-server/yandex-tracker` (успешно ✅)
- [x] Все тесты проходят

---

## Ожидаемый результат

✅ Функция `calculateToolsMetrics()` реализована
✅ Детальное логирование при `tools/list`
✅ Метрики: total, size, categories, priorities
✅ Предупреждения для оптимизации
✅ Тесты покрывают метрики
✅ Логи читаемы и информативны

---

## Пример лога (ожидаемый вывод)

```
[INFO] 📋 Запрос tools/list от клиента
[INFO] ✅ Возвращаем 11 инструментов (режим: eager) {
  totalTools: 11,
  mode: 'eager',
  descriptionLength: 450,
  estimatedTokens: 113
}
[DEBUG] 📊 Распределение инструментов {
  byCategory: { issues: 8, helpers: 2, system: 1 },
  byPriority: { critical: 5, high: 3, normal: 2, low: 1 },
  bySubcategory: { read: 3, write: 2, workflow: 2, url: 1, demo: 1, health: 1 }
}
[DEBUG] 🔢 Порядок инструментов: [
  { name: 'create_issue', category: 'issues', priority: 'critical' },
  { name: 'find_issues', category: 'issues', priority: 'critical' },
  { name: 'get_issues', category: 'issues', priority: 'critical' },
  ...
]
```

---

## Следующий этап

→ **4.1_documentation.md** — документировать best practices
