...
- [x] Временные файлы удалены (не было)
- [x] Неиспользуемые imports удалены
- [x] Legacy код СОХРАНЕН в `cli-legacy/`
- [x] README обновлен с упоминанием миграции
- [x] Legacy тесты перемещены в `tests/cli-legacy/` и исключены из vitest
- [x] `npm run validate` проходит (142 теста успешно)
...
- [x] Lint проходит без предупреждений
- [x] Legacy CLI доступен для отката
- [x] Структура чистая и понятная
