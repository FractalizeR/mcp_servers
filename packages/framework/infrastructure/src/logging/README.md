...
```
logs/
├── combined.log                # Все логи текущего дня (JSON)
├── 20251119-combined.log.gz    # Вчерашние логи (архив)
├── 20251118-combined.log.gz    # Позавчерашние логи
├── ...
├── error.log                   # Только error (текущий день)
├── 20251119-error.log.gz       # Вчерашние ошибки (архив)
└── ...
```

**Ротация:**
- Ежедневно в полночь (дата добавляется в имя файла)
- ИЛИ при достижении максимального размера (`LOG_MAX_SIZE`)
