 ...
 * зная его имя.
 *
 * Решение: ОДИН объект, который спрашивают обе точки:
 * - `ToolRegistry.getDefinitions(disabledFilter)` — что показать в tools/list
 *   (через ToolFilterService.applyDisabledFilter);
 * - `ToolRegistry.execute(...)` — можно ли исполнить конкретный tool.
 *
 * Оба пути используют ОДИН И ТОТ ЖЕ статический предикат
 * `ToolFilterService.isDisabledByFilter`, поэтому решение о видимости и
 * решение о исполняемости не могут разойтись: это гарантия на уровне
 * используемой функции, а не на уровне совпадения конфигурации в двух
 * разных местах.
 *
