 ...
 * Живой пример: инструмент завершения задачи делает `GET
 ...
 * поведение). Эмпирически проверено на write-инструментах всех
 * серверов, делающих >1 HTTP-вызов (например, Wiki: `update_page`) — во ВСЕХ случаях
...
export function describeUnreachableLeaf(toolName: string, unreachable: UnreachableLeaf): string {
  ...
  if (foundInNonTargetCall) {
    return (
      ...
      'параметр доезжал до подготовительного GET, но не до целевого POST). Если это ложное ' +
