 ...
 *     const tool = new GetProjectTool(mockFacade, mockLogger);
 *     const definition = tool.getDefinition();
 *
 *     // Проверяем, что definition соответствует schema
 *     expectDefinitionMatchesSchema(definition.inputSchema, GetProjectSchema);
 *   });
 * });
 * ```
 */
 ...
 *   expectDefinitionFullyValid(definition.inputSchema, GetProjectSchema);
 * });
 * ```
 */
