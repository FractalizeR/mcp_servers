...
**Покрытие: `npm run coverage:check` (входит в корневую `validate`):**
- сверяет `tests/COVERAGE_MATRIX.md` со сгенерированной — расхождение чинится
  `npm run coverage:matrix`, а не правкой файла
- держит **храповик дыр**: `tests/coverage-exceptions/coverage-gate-baseline.ts` —
  замороженный НАБОР пар «инструмент × свойство» (не число: скаляр не отличает новую
  дыру от унаследованной). Отказ **в обе стороны** — новая дыра роняет, закрытая и не
  убранная из набора тоже роняет. Легитимна правка «убрал строку вместе с появлением
  теста»; **дописывать строку нельзя**, кроме одного случая — снятого живого наблюдения,
  и только вместе с записью `RetiredLiveObservation` на ту же пару. Проверяется машинно:
  каждый ключ базлайна сверяется с замороженным снимком `COVERAGE_GATE_BASELINE_ORIGIN`
  (снимок закрыт отпечатком), ключ вне снимка без записи о снятии роняет прогон. Строка
  нужна только для свойств гейта (`GATED_PROPERTIES`): снятие наблюдения по **С-5**
  клетки гейта не создаёт, и строка `tool[С-5]` в базлайне сама становится нарушением
- живьём наблюдённое пишется потульно в `tests/coverage-exceptions/live-observations.ts`
  со ссылкой на отчёт в `tests/live-runs/` (имя отчёта — `{N}_LIVE_RUN_REPORT_{дата}.md`,
  конвенция нормативна); критерий записи — **эффект прочитан обратно**, код ответа
  наблюдением не является; каждая запись несёт `schemaFingerprint` — отпечаток схемы
  параметров инструмента, и правка схемы роняет проверку с тремя выходами: перепроверить
  живьём, пере-штамповать запись (`fingerprintRestamp`) или снять наблюдение
  (`RetiredLiveObservation`). Отпечаток **не видит ни маршрут** (он в операции API), **ни
  форму ответа** (`outputSchema` и DTO разбора в него не входят, а С-5 читается именно
  через ответ) — оговорка в [tests/TESTING_STRATEGY.md](tests/TESTING_STRATEGY.md) §1
