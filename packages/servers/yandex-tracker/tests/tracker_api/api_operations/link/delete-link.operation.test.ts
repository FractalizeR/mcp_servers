...
describe('DeleteLinkOperation', () => {
  ...
  describe('executeMany', () => {
    it('should delete multiple links in parallel', async () => {
      ...
      expect(at(result).key).toBe('TEST-1:link1');
      expect(at(result, 1).status).toBe('fulfilled');
      expect(at(result, 1).key).toBe('TEST-2:link2');
      expect(at(result, 2).status).toBe('fulfilled');
      expect(at(result, 2).key).toBe('TEST-3:link3');
    ...
    it('should delete links from same issue with different link IDs', async () => {
      ...
      expect(at(result).key).toBe('TEST-100:link1');
      expect(at(result, 1).key).toBe('TEST-100:link2');
      expect(at(result, 2).key).toBe('TEST-100:link3');
    });
