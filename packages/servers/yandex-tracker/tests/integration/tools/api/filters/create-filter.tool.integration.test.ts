/**
 * С-4/С-5 у `create_filter` в матрице — `живьём`: реестр
 * (`tests/coverage-exceptions/live-observations.ts`) несёт запись по прогону
 * `sweep7-2026-08-26` — фильтр `7` создан и прочитан прямым `GET /v3/filters/7`. Клетку производит реестр, а не этот
