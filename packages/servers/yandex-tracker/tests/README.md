...
1. **Неполные моки с `as unknown as`**
   ```typescript
   // ❌ ПЛОХО (без mock factory)
...
1. **Spy на приватные методы (стандартная практика тестирования)**
   ```typescript
   // ✅ ДОПУСТИМО с eslint-disable комментарием
   // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Spy на приватное поле для проверки вызовов
   const spy = vi.spyOn(logger['pino'] as any, 'info');
   ```
...
2. **Замена приватных полей для unit тестирования**
   ```typescript
   // ✅ ДОПУСТИМО если DI потребует изменения production кода
   // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Замена приватного поля для unit тестирования
   (operation as any).parallelExecutor = mockExecutor;
   ```

3. **Намеренно невалидные значения для тестов валидации**
   ```typescript
   // ✅ ДОПУСТИМО для проверки обработки ошибок
   // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Намеренно невалидное значение для теста валидации
   const result = await tool.execute({ invalid: 'value' as any });
   ```

4. **Доступ к опциональным полям для проверки**
   ```typescript
   // ✅ ДОПУСТИМО для проверки метаданных
...
**ВАЖНО:** Каждое использование `as any` ДОЛЖНО быть сопровождено комментарием `eslint-disable-next-line` с объяснением необходимости.
