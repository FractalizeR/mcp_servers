...
export interface FindIssuesInputDto {
   ...
   *
   * Кодирует next-путь предыдущего ответа + ХЕШ канонического тела запроса (R2).
   * Критерии поиска (query/filter/keys/queue/filterId/order) ОБЯЗАНЫ быть переданы
   * повторно вместе с курсором: операция канонизирует их, считает хеш и сверяет
   * с хешем в курсоре (несовпадение → explicit error, fail-fast).
   *
   * Несовместим с perPage/fetchAll/maxItems (зафиксированы внутри курсора).
   */
