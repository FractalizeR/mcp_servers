 ...
 * (пакет 6.1 плана модернизации MCP 2026-07-28, «анализ задачи и правка
 * description»). В отличие от `IssueResourceProvider`/`QueueResourceProvider`
...
import { ISSUE_DESCRIPTION_EDITOR_HTML } from './issue-description-editor.widget.js';
...
export class IssueDescriptionEditorResourceProvider implements ResourceProvider {
  ...
  readResource(uri: string): readonly McpResourceContents[] | undefined {
    ...
    const content: McpResourceContents & { _meta: UiCspMeta } = {
      uri,
      mimeType: MCP_APP_MIME_TYPE,
      text: ISSUE_DESCRIPTION_EDITOR_HTML,
      _meta: EMPTY_CSP,
    };
    return [content];
  }
