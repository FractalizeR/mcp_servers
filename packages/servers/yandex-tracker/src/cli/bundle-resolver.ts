/**
 * Резолвер пути к собранному бандлу MCP сервера.
 *
...
const PACKAGE_BUNDLE_SPECIFIER =
  '@fractalizer/mcp-server-yandex-tracker/dist/yandex-tracker.bundle.cjs';

/**
 * Дефолтный резолвер бандла Yandex Tracker.
 *
 * Алгоритм:
 ...
 *     `'../../yandex-tracker.bundle.cjs'` приводит к корню `dist/`. ESM-корректная
...
export const defaultBundleResolver: BundleResolver = (): string => {
  ...
  const fallbackPath = fileURLToPath(new URL('../../yandex-tracker.bundle.cjs', import.meta.url));
  if (fs.existsSync(fallbackPath)) {
    return fallbackPath;
  }

  throw new Error(
    'Не удалось найти бандл MCP сервера Yandex Tracker.\n' +
