/**
 * Доменные проверки `doctor` для Yandex Wiki MCP сервера.
 *
      ...
      try {
        ...
        return {
          ...
          details: message
            ...
            .map((s) => s.trim())
            .filter(Boolean),
          hint: 'Соберите пакет (`npm run build`) или переустановите глобально (`npm install -g @fractalizer/mcp-server-yandex-wiki`).',
        };
      }
    },
  };
      ...
      try {
        ...
        if (isENOENT(err)) {
          return {
            ...
            hint: 'Запустите `mcp-yandex-wiki-connect connect`, чтобы сохранить конфигурацию.',
          };
        }
        return {
          status: 'fail',
...
/**
 * Агрегатор доменных проверок Yandex Wiki для команды `doctor`.
 */
export function getYwDoctorChecks(): DoctorCheck[] {
