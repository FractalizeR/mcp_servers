...
/**
 * Читает версию из package.json yandex-wiki пакета
 */
async function getPackageVersion(): Promise<string> {
  ...
  const packageJsonPath = isInWorkspace
    ...
    : path.join(projectRoot, 'packages/servers/yandex-wiki/package.json');
...
async function setBuildHash(): Promise<void> {
  // Определяем корень пакета (packages/servers/yandex-wiki)
  const projectRoot = path.resolve(process.cwd());
  const isInWorkspace = projectRoot.includes('packages/servers/yandex-wiki');
