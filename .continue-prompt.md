# Промт для продолжения работы над fyt-mcp

## Контекст проекта

Это **fyt-mcp** (FractalizeR's Yandex Tracker MCP) — MCP сервер для интеграции с API Яндекс.Трекера v3.

### Что уже сделано (последние изменения)

1. **Ребрендинг проекта:**
   - `yandex-tracker-mcp` → `fyt-mcp`
   - Все инструменты переименованы: `yandex_tracker_*` → `fyt_mcp_*`
...
2. **CLI инструмент для управления подключениями:**
   ...
   - Документация: `docs/CLI.md`, `docs/cli-architecture.md`
...
2e59780 feat: ребрендинг проекта в fyt-mcp + CLI инструмент для управления подключениями
```
...
- [ ] Протестировать установку: `npm pack` → `npm install -g fyt-mcp-*.tgz`
- [ ] Опубликовать: `npm publish --access public`

**Scoped package:**
```json
{
  "name": "@fractalizer/fyt-mcp",
  "bin": {
    "fyt-mcp": "./dist/cli/bin/mcp-connect.js"
  }
}
```
...
- `docs/cli-architecture.md` — полная архитектура
...
- **Проект:** https://github.com/fractalizer/fyt-mcp
- **Документация:** `README.md`, `docs/CLI.md`, `ARCHITECTURE.md`
- **Конвенции:** `CLAUDE.md`, `src/mcp/CONVENTIONS.md`
